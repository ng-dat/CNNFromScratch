...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        # print('dprev', dprev.shape, 'meta',self.meta.shape, 'W', self.params[self.w_name].shape, 'bias', self.params[self.b_name].shape)
        dfeat = dprev @ self.params[self.w_name].T
        self.grads[self.w_name] = self.meta.T  @ dprev  # Todo me: check here should use += or :=
        self.grads[self.b_name] = np.sum(dprev, axis=0) # Todo me: check here should use += or :=
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class leaky_relu(object):
    ...
    def forward(self, feat):
        ...
        #############################################################################
        output = feat.copy()
        output[feat < 0] *= self.negative_slope
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = feat
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        dfeat = dprev.copy()
        dfeat[self.meta<0] *= self.negative_slope
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class dropout(object):
    ...
    def forward(self, feat, is_training=True, seed=None):
        ...
        if is_training and self.keep_prob != 0:
            kept = self.rng.choice([0, 1], size=feat.shape, p=[1-self.keep_prob, self.keep_prob])
            output = feat * kept
            output *= 1.0/self.keep_prob
        else:
    ...
    def backward(self, dprev):
        ...
        if self.is_training and self.keep_prob != 0:
            dfeat *= self.kept /self.keep_prob
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.is_training = False
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        # print('label', label.shape, 'logit', logit.shape)
        N = label.shape[0]
        log_likelihood = -np.log(logit[range(N), label])
        loss = np.sum(log_likelihood)
        if self.size_average:
            #loss = -np.mean(one_hot_label @ np.log(logit).T) # loss = -np.mean(np.sum(one_hot_label * np.log(logit), axis=1))
            loss /= N
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
...
def softmax(feat):
    ...
    #############################################################################
    def softmax_in_vector(z):
        e_z = np.exp(z)
        return e_z / e_z.sum()
    scores = np.apply_along_axis(softmax_in_vector, -1, feat)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    return scores
